...
namespace DroplerGUI.Services.Steam
{
	public class MobileAuth
	{
		...
		{
			return GenerateSteamGuardCodeForTime(TimeAligner.GetAlignedTime());
		}
		...
		public Confirmation[] FetchConfirmations()
		{
			...
			string response = SteamWeb.Request(url, "GET", new NameValueCollection(), cookies);
		...
		public async Task<Confirmation[]> FetchConfirmationsAsync()
		{
			...
			string response = await SteamWeb.RequestAsync(url, "GET", new NameValueCollection(), cookies);
		...
		{
			if ((int)conf.ConfType != (int)Confirmation.ConfirmationType.Trade)
		...
		private ConfirmationDetailsResponse _getConfirmationDetails(Confirmation conf)
		{
			...
			string response = SteamWeb.Request(url, "GET", new NameValueCollection(), cookies, null);
		...
		private bool _sendMultiConfirmationAjax(Confirmation[] confs, string op)
		{
			...
			{
				query.Add("cid[]", conf.ID.ToString());
				query.Add("ck[]", conf.Key.ToString());
			}
			...
			string response = SteamWeb.Request(url + queryString, "POST", query, cookies, null);
			if (response == null) return false;
		...
		public NameValueCollection GenerateConfirmationQueryParamsAsNVC(string tag)
		{
			...
			long time = TimeAligner.GetAlignedTime();
